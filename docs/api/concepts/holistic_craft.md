...
Holistic CRAFT follows the same core principle as CRAFT but operates on activation maps:
...
!!!info
    For models with global average pooling, provide the layer **before** pooling to preserve spatial information needed for concept localization.

!!!warning
    Activations must be non-negative (use ReLU or similar activation functions before the extraction layer).
...
For a complete working example, see the [YOLOX CRAFT notebook](https://github.com/deel-ai/xplique/blob/master/examples/torch_yolox_craft_coco_clean.ipynb).
