...
def test_box_xyxy_to_xywh():
    ...
    xyxy_boxes = tf.constant([[40, 40, 60, 60], [0.25, 0.25, 0.35, 0.35]], dtype=tf.float32)
    xywh_boxes = TfBoxManager.box_xyxy_to_xywh(xyxy_boxes)
    expected_boxes = tf.constant([[40, 40, 20, 20], [0.25, 0.25, 0.1, 0.1]], dtype=tf.float32)
    assert tf.reduce_all(tf.abs(xywh_boxes - expected_boxes) < 1e-6)
...
def test_translator_fcos_pixel_xyxy_to_normalized_xyxy():
    ...
    output_boxes = translator.translate(input_boxes, image_size=image_size)
