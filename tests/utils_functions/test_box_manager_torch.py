...
def test_box_xyxy_to_xywh():
    ...
    xyxy_boxes = torch.tensor([[40, 40, 60, 60], [0.25, 0.25, 0.35, 0.35]], dtype=torch.float32)
    xywh_boxes = TorchBoxManager.box_xyxy_to_xywh(xyxy_boxes)
    expected_boxes = torch.tensor([[40, 40, 20, 20], [0.25, 0.25, 0.1, 0.1]], dtype=torch.float32)
    assert torch.allclose(xywh_boxes, expected_boxes)
