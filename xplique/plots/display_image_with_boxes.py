...
from xplique.utils_functions.object_detection.tf.box_formatter import (
    MultiBoxTensor as TfMultiBoxTensor,
)
...
try:
    from xplique.utils_functions.object_detection.torch.box_formatter import (
        MultiBoxTensor as TorchMultiBoxTensor,
    )
...
def _make_translator(multibox_results, box_type):
...
def display_image_with_boxes(
    ...
    # Build a framework-aware translator: input box_type -> XYXY non-normalized
    ...
    for box_coords, score, proba in zip(boxes, scores, probas):
        # Translate single box (add/remove batch dim around the (4,) vector)
        translated = box_translator.translate(box_coords[np.newaxis], image_size=image.size)
