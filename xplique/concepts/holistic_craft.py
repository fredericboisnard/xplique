...
class PartialExplainer:
    ...
    **kwargs
        Configuration arguments for the explainer (e.g., operator, harmonize, grid_size).
...
class HolisticCraft:
    ...
    def display_images_per_concept(
        ...
        for i, c_i in enumerate(concepts_id):
            for image_id, image in enumerate(images):
                ...
                show_ax(heatmap, cmap=self.cmaps[::-1][cmap_idx], alpha=0.5, ax=axs[image_id, i])
        return fig
    ...
    def display_top_images_per_concept(
        ...
        for i, c_i in enumerate(concepts_id):
            ...
            for j, image_id in enumerate(top_image_for_concept_i[:topk]):
        ...
        **method_kwargs
        ...
        if method == "gradient_input":
            explainer = PartialExplainer(
                GradientInput, operator=operator, harmonize=False, **method_kwargs
            )
        elif method == "sobol":
        ...
        explanation = self.compute_explanation_per_concept(images, explainer, class_id, confidence)
